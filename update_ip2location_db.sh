#!/bin/bash
# Скачивание IP2Location LITE (ZIP) на platformExp (161.97.96.43).
...
#   ./update_ip2location_db.sh --only db11|asn|all
#
...
case "$ONLY" in
    db11|asn|all) ;;
    *) echo "✗ --only: db11, asn или all" >&2; exit 1 ;;
esac
...
if [[ "$DRY_RUN" -eq 0 && "$FORCE" -eq 0 ]]; then
    ...
    read -p "Скачать IP2Location LITE (only=$ONLY)? (yes/no): " confirm
    [[ "$confirm" == "yes" ]] || { echo "Отменено."; exit 1; }
fi

log "=========================================="
log "IP2Location download (only=$ONLY, staging=$TARGET_DIR)"
